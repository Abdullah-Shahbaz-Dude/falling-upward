...
export function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const heroHeight = isHomePage ? window.innerHeight : window.innerHeight * 0.75; // 75vh for other pages
  ...
  return (
    <header
      className={`fixed w-full z-50 transition-all duration-300 ${
        ...
        isHeroVisible && !isScrolled ? 'absolute top-0' : shouldShowNavbar ? 'fixed top-0' : 'fixed -top-24'
      }`}
    >
      <div className="container-custom mx-auto flex justify-between items-center bg-transparent">
        ...
        <nav className="hidden md:flex items-center space-x-8">
                ...
                <div 
                  className={`flex items-center cursor-pointer font-medium transition-colors ${
                    isActive(link.href) || pathname.includes(link.href)
                      ? isScrolled ? 'text-[#0B4073] border-b-2 border-[#7094B7]' : 'text-[#D6E2EA] border-b-2 border-[#D6E2EA]'
                      : isScrolled ? 'text-gray-700 hover:text-[#0B4073]' : 'text-white hover:text-[#D6E2EA]'
                  }`}
                >
                  {link.name}
                  <FiChevronDown className={`ml-1 transition-transform duration-300 ${isOffersOpen ? 'rotate-180' : ''}`} />
                </div>
                  ...
                  {isOffersOpen && (
                    ...
                    >
                      ...
                      {link.dropdownItems?.map((item) => (
                        ...
                        >
                          <div>
                            <span className="font-medium group-hover:text-[#0B4073] transition-colors">{item.name}</span>
                          </div>
                        </Link>
                      ))}
                    </motion.div>
          ...
          <Link
            href="/book"
            className={`${isScrolled ? 'btn-primary bg-[#7094B7] hover:bg-[#0B4073] text-white' : 'btn-outline border-white text-white hover:bg-white/10'} mr-4`}
          >
            Book a Consultation
          </Link>
          <Link
            href="/login"
            className={`${isScrolled ? 'btn-primary bg-[#0B4073] hover:bg-[#083258] text-white' : 'btn-outline border-white text-white hover:bg-white/10'}`}
          >
            Login
          </Link>
        </nav>
            ...
            {navLinks.map((link) => (
              link.hasDropdown ? (
                <div key={link.name} className="flex flex-col">
                  ...
                  >
                    {link.name}
                    <FiChevronDown className={`transition-transform duration-300 ${isOffersOpen ? 'rotate-180' : ''}`} />
                  </div>
                    ...
                    {isOffersOpen && (
                      ...
                      >
                        <div className="ml-4 mt-2 border-l-2 border-[#7094B7] pl-4 py-2">
                          <h3 className="text-xs font-semibold text-gray-500 px-4 pb-2">Our Services</h3>
                          {link.dropdownItems?.map((item) => (
                            <Link
                              key={item.name}
                              href={item.href}
